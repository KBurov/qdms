...
{
    public class Level1SummaryUpdateEventArgs : System.EventArgs
    {
    ...
    public class Level1FundamentalEventArgs : System.EventArgs
    {
    ...
    public class IQLevel1WatchItem
    {
        ...
        {
            if (_socket == null) {
                throw new Exception("Watch Item not connected");
            }
            if (_active) {
                return;
            }
        ...
        public void SetRegionalOn()
        {
            ...
            if (!_active) {
                throw new Exception("Cannot set Regional Quotes On, Activate first");
            }
            if (_regionOn) {
                return;
            }
        ...
        public void RequestFundamental()
        {
            ...
            if (!_active) {
                throw new Exception("Cannot request Fundamental data, Activate first");
            }
    ...
    public class IQLevel1Client : SocketClient, IList<IQLevel1WatchItem>
    {
        // Delegates for event
        ...
        public bool isNewsOn { get { return _newsOn; } }

        // Implementing IList<IQLevel1WatchItem> interface
        public int IndexOf(IQLevel1WatchItem item)
        {
            if (!_key.ContainsKey(item.Symbol)) return -1;
            return _key[item.Symbol];
        }

        public void Insert(int index, IQLevel1WatchItem item)
        {
            throw new Exception("Cannot Insert, Use Add");
        }

        public void RemoveAt(int index)
        {
            throw new Exception("Cannot Remove, use Deactivate or Clear all symbols");
        }

        public IQLevel1WatchItem this[int index] { get { return _watchList[index]; } set { throw new Exception("The list is Read Only"); } }

        // Implementing ICollection<IQLevel1WatchItem
        public void Add(IQLevel1WatchItem item)
        {
            if (_key.ContainsKey(item.Symbol)) {
                throw new Exception("Symbol (" + item.Symbol + ") already exists");
            }
            _key.Add(item.Symbol, _watchList.Count);
            _watchList.Add(item);
            item.Connect(this);
        }
        ...
        {
            Send("S,UNWATCH ALL\r\n");
            foreach (IQLevel1WatchItem wi in _watchList) {
                wi.Disconnect();
            }
            _key.Clear();
            _watchList.Clear();
        ...
        public bool Contains(IQLevel1WatchItem item)
        {
            return _key.ContainsKey(item.Symbol);
        }

        public void CopyTo(IQLevel1WatchItem[] array, int arrayIndex)
        {
            _watchList.CopyTo(array, arrayIndex);
        }

        public int Count { get { return _watchList.Count; } }
        public bool IsReadOnly { get { return false; } }

        public bool Remove(IQLevel1WatchItem item)
        {
            throw new Exception("Remove no allowed, use Deactivate");
        }

        // Implementing IEnumerable<IQLevel1WatchItem> & System.Collections.IEnumerable interfaces
        public IEnumerator<IQLevel1WatchItem> GetEnumerator()
        {
        ...
        public IQLevel1WatchItem this[string symbol] { get { return _watchList[_key[symbol]]; } }
        ...
        protected override void OnTextLineEvent(TextLineEventArgs e)
        {
            ...
            throw new Exception("(Level 1) NOT HANDLED:" + e.textLine);
        }
        ...
        #region private
        private List<IQLevel1WatchItem> _watchList;
        private Dictionary<string, int> _key;
        private bool _newsOn;
        #endregion
    }
}